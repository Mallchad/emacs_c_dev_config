...
local arg_verbs =
   {
      build     = "<replace me> regenerate the build system and compile the project",
      clean     = "<replace me> clean up irrelevant temporary files",
      configure = "<replace me> run local-only initilization and setup for files configs",
      run       = "run the primary executable or target from the project",
...
local arg_variables =
   {
      build_type =
         {
            [[A string that is a passed to cmake as FERING_BUILD_TYPE by default,
...
local function unsafe_begin(operation_description)
   ...
   elseif dry_run then
      log("--dry-run specified, directories will not be rmeoved")
   end
end
...
if arg_parsed.clean_only ~= true then
   ...
   os.execute(build_binary_string)
   print("[Finishing Up]")
   print("[Done]")
   print("")                  -- Just command line padding
end
...
-- If you want to configure the tool you should probably start here.
...
local function regenerate_variables()
   ...
   -- If you want a clean slate, you should uncomment some of the following lines
   -- to clear out default values.
   -- arg_verbs = {}            -- Suggest implimentat least default verbs
   -- arg_flags = {}            -- Suggest impliment at least UNSAFE/dry_run
   -- arg_flags = {}
   -- local_dirs = {}           -- Suggest implment at least build/build_binaries
   ...
   averb.build     = "regenerate the build system and compile the project"
   averb.clean     = "remove local, non-critical untracked files where possible"
   averb.configure = "setup desirable local-only files and build enviornment"
   averb.run       = "run the an executable from the project"
   averb.help      = "display help string"
